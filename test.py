...
vol = mesh.binarize(spacing=(0.001,0.001,0.005))
pts = vol.points
...
vol.pointdata["Shear Stress"] = shear_vals
vol.cmap(cmap, shear_vals, vmax=np.max(shear_vals), vmin=np.min(shear_vals))
vol.alpha(1)
vol.add_scalarbar(title="Shear Stress (Pa)", c="w")

# ----------------- First Plot: Just Nozzle -----------------
plt1 = Plotter(title="3D Shear Field", size=(900, 700), axes=1, bg="k")
plt1.show(vol, zoom=1.2, viewup="z", interactive=True)
